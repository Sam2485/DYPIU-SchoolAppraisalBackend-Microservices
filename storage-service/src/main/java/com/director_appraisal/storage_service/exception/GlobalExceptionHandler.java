package com.director_appraisal.storage_service.exception;

import com.director_appraisal.storage_service.config.MdcLoggingFilter;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleFileNotFound(FileNotFoundException e, HttpServletRequest req) {
        log.warn("[FILE_NOT_FOUND] path={} message={}", req.getRequestURI(), e.getMessage());
        return build(HttpStatus.NOT_FOUND, "FILE_NOT_FOUND", e.getMessage(), req, null);
    }

    @ExceptionHandler(MaxUploadSizeExceededException.class)
    public ResponseEntity<ApiErrorResponse> handleMaxUploadSize(MaxUploadSizeExceededException e, HttpServletRequest req) {
        log.warn("[PAYLOAD_TOO_LARGE] path={} message={}", req.getRequestURI(), e.getMessage());
        return build(HttpStatus.PAYLOAD_TOO_LARGE, "ATTACHMENT_TOO_LARGE", "File size exceeds the 25MB maximum limit.", req, null);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleGeneralException(Exception e, HttpServletRequest req) {
        ...
        return build(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_SERVER_ERROR",
                "File operation failed. Please try again or contact support.", req, null);
    }
