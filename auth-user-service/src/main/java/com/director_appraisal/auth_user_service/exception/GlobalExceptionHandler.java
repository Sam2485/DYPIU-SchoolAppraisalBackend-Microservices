package com.director_appraisal.auth_user_service.exception;

import com.director_appraisal.auth_user_service.config.MdcLoggingFilter;
...
public class GlobalExceptionHandler {
    ...
    private static final String SERVICE_NAME = "auth-user-service";

    @ExceptionHandler(BadCredentialsException.class)
    public ResponseEntity<ApiErrorResponse> handleBadCredentials(BadCredentialsException e, HttpServletRequest req) {
        log.warn("[AUTH_FAILED] path={} message={}", req.getRequestURI(), e.getMessage());
        return build(HttpStatus.UNAUTHORIZED, "AUTH_INVALID_CREDENTIALS", "Invalid email or password.", req, null);
    }

    @ExceptionHandler(AccessDeniedException.class)
    public ResponseEntity<ApiErrorResponse> handleAccessDenied(AccessDeniedException e, HttpServletRequest req) {
        log.warn("[ACCESS_DENIED] path={} message={}", req.getRequestURI(), e.getMessage());
    ...
    public ResponseEntity<ApiErrorResponse> handleDataIntegrity(DataIntegrityViolationException e, HttpServletRequest req) {
        ...
        return build(HttpStatus.CONFLICT, "DATABASE_CONSTRAINT_VIOLATION", "Database constraint or duplicate key violation.", req, null);
    }
